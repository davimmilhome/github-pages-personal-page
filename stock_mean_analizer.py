...
def actual_price(ticket):
    ...
    v_actual_price = round(ticket_df["Adj Close"].iloc[-1],2)

    #Médias
    df_mean_6m = ticket_df["Adj Close"].tail(180)
    v_mean_6m = round(df_mean_6m.mean(), 2)

    df_mean_12m = ticket_df["Adj Close"].tail(360)
    v_mean_12m = round(df_mean_12m.mean(), 2)

    df_mean_36m = ticket_df["Adj Close"].tail(1080)
    v_mean_36m = round(df_mean_36m.mean(), 2)

    df_mean_60m = ticket_df["Adj Close"].tail(1800)
